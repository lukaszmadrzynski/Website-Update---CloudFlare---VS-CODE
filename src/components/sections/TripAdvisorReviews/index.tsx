...
function ReviewCard({ review }) {
    ...
    return (
        <div className="bg-white rounded-xl shadow-md border border-gray-200 p-4 md:p-5 flex-shrink-0" style={{ width: CARD_WIDTH, marginLeft: CARD_MARGIN, marginRight: CARD_MARGIN }}>
...
export default function TripAdvisorReviews({ reviewCount = 9 }) {
    const reviews = [
        {
            name: 'Shirley O',
            date: 'April 2025',
    ...
    const scrollAmount = CARD_WIDTH + (CARD_MARGIN * 2);
    const totalCards = reviews.length;
    const originalSetEnd = totalCards * scrollAmount; // End of original reviews
    const isTransitioning = React.useRef(false);

    // Auto-scroll with seamless loop reset
    React.useEffect(() => {
        if (isPaused || !scrollRef.current || isTransitioning.current) return;
        ...
        const interval = setInterval(() => {
            if (scrollRef.current && !isTransitioning.current) {
                ...
                // Check if we're in the original set or duplicates
                const inOriginalSet = scrollLeft < originalSetEnd;
                
                if (inOriginalSet) {
                    // Normal forward scroll
                    nextPosition = scrollLeft + scrollAmount;
                    nextIndex = currentIndex + 1;
                    
                    // If reaching end of original set, jump to duplicate
                    if (nextPosition >= originalSetEnd) {
                        // Wait for transition to end, then reset to beginning
                ...
                if (!isTransitioning.current) {
                    ...
                    setCurrentIndex(nextIndex % totalCards);
                }
            }
        }, 4000); // 4 seconds per card
        
        return () => clearInterval(interval);
    }, [isPaused, currentIndex, scrollAmount, totalCards, originalSetEnd]);
    ...
    React.useEffect(() => {
        ...
        const handleScroll = () => {
            if (scrollRef.current) {
                ...
                // Map duplicate positions back to original
    ...
    const handleScrollLeft = () => {
        if (scrollRef.current) {
            ...
            // If at beginning, go to last duplicate position for seamless wrap
            if (scrollLeft <= scrollAmount) {
                newPosition = totalCards * scrollAmount; // Last duplicate
            } else {
    ...
    const handleScrollRight = () => {
        if (scrollRef.current) {
            ...
            // If at end of original set, jump to beginning
            if (scrollLeft >= originalSetEnd - scrollAmount) {
                newPosition = 0; // Jump to start
            } else {
                newPosition = scrollLeft + scrollAmount;
            }
    ...
    return (
        <div className="w-full py-12 px-4">
            {/* Section Title - matching other section titles */}
            <h2 className="sb-component sb-component-title text-dark text-center mb-8">
            ...
            <div className="flex items-center justify-center gap-4 mx-auto py-5" style={{ maxWidth: '1070px' }}>
                {/* Left Arrow */}
                <button
                    onClick={handleScrollLeft}
                    onMouseEnter={() => setIsPaused(true)}
                    onMouseLeave={() => setIsPaused(false)}
                    className="flex-shrink-0 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white rounded-full shadow-lg hover:bg-gray-50 transition-all duration-1000"
                ...
                <div 
                    ref={scrollRef}
                    className="flex overflow-x-auto flex-1 py-4"
                    onMouseEnter={() => setIsPaused(true)}
                    onMouseLeave={() => setIsPaused(false)}
                    style={{ 
                ...
                >
                    {reviews.map((review, index) => (
                        <ReviewCard key={`orig-${index}`} review={review} />
                    ))}
